...
const ProfileOrderDetails = () => {
  ...
  const { data: getUserOrderData } = useGetUserOrderQuery({
  ...
  return (
    <section className="my-10 sm:w-[80%] border border-1 border-gray-300">
      <div className="flex items-center justify-between border-b border-gray-300 sm:p-4 p-2">
        <h1 className="flex items-center sm:gap-2 gap-1 uppercase font-semibold">
          <IoMdArrowBack className="text-lg" />
          Order Details
        </h1>
        <span className="text-orange-500 text-lg">
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger className="text-base">
              Leave a Rating +
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription className="space-y-3">
                    ...
                    <SelectContent>
                      <SelectItem value={5}>
                        <div className="flex items-center gap-1">
                          {generateStars(5)}5 Star Rating
                        </div>
                      </SelectItem>
                      <SelectItem value={4}>
                        <div className="flex items-center gap-1">
                          {generateStars(4)}4 Star Rating
                        </div>
                      </SelectItem>
                      <SelectItem value={3}>
                        <div className="flex items-center gap-1">
                          {generateStars(3)}3 Star Rating
                        </div>
                      </SelectItem>
                      <SelectItem value={2}>
                        <div className="flex items-center gap-1">
                          {generateStars(2)}2 Star Rating
                        </div>
                      </SelectItem>
                      <SelectItem value={1}>
                        <div className="flex items-center gap-1">
                          {generateStars(1)}1 Star Rating
                        </div>
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  ...
                  />
                  <Button variant="shop" onClick={handleAddProductFeedback}>
                    Publish Review
                  </Button>
                </DialogDescription>
              </DialogHeader>
        ...
        <section>
          <div className="sm:flex items-center justify-between bg-yellow-100 sm:p-4 p-2 sm:m-5 m-2">
            <div>
              ...
              <div className="sm:flex items-center gap-2">
                ...
                <p>
                  Order Placed in{" "}
                  {new Date(userOrderData[0]?.createdAt).toLocaleDateString()}{" "}
                  at{" "}
                  {new Date(userOrderData[0]?.createdAt).toLocaleTimeString()}
                </p>
              </div>
            </div>
            <span className="text-xl font-semibold text-blue-400">
              ${userOrderData[0]?.totalPrice}
            </span>
          </div>
          ...
          <div className="sm:p-10 p-2 sm:px-24 border-b border-gray-300">
            ...
            <div className="flex items-center justify-between mt-4">
              <p className="flex flex-col items-center">
                <PiNotebookBold
                  className={`text-2xl ${
                    progressValue >= 0 ? "text-green-600" : "text-green-300"
                  }`}
                />
                <span
                  className={
              ...
              <p className="flex flex-col items-center">
                <PiPackageDuotone
                  className={`text-2xl ${
                    progressValue >= 33 ? "text-orange-600" : "text-orange-300"
                  }`}
                />
                <span
                  className={
              ...
              <p className="flex flex-col items-center">
                <PiTruckDuotone
                  className={`text-2xl ${
                    progressValue >= 66 ? "text-orange-600" : "text-orange-300"
                  }`}
                />
                <span
                  className={
              ...
              <p className="flex flex-col items-center">
                <PiHandshake
                  className={`text-2xl ${
                    progressValue >= 100 ? "text-orange-600" : "text-orange-300"
                  }`}
                />
                <span
                  className={
          ...
          <div className="space-y-2 mt-4 px-5">
            {userOrderData[0]?.orderStatus === "delivered" && (
              <p className="flex items-center gap-2">
                <span className="bg-green-100 p-2">
                  <PiChecks className="text-2xl text-green-700" />
                </span>
            ...
            {userOrderData[0]?.orderStatus === "shipped" && (
              <p className="flex items-center gap-2">
                <span className="bg-sky-200 p-2">
                  <PiUser className="text-2xl text-blue-500" />
                </span>
            ...
            {userOrderData[0]?.orderStatus === "out_of_delivery" && (
              <>
                <p className="flex items-center gap-2">
                  <span className="bg-sky-200 p-2">
                    <PiMapPinLine className="text-2xl text-blue-500" />
                  </span>
                  Your order has reached at last mile hub.
                </p>
                <p className="flex items-center gap-2">
                  <span className="bg-sky-200 p-2">
                    <PiMapTrifold className="text-2xl text-blue-500" />
                  </span>
            ...
            {userOrderData[0]?.orderStatus === "completed" && (
              <>
                ...
                <p className="flex items-center gap-2">
                  <span className="bg-sky-200 p-2">
                    <PiNotepad className="text-2xl text-blue-500" />
                  </span>
          ...
          <h3 className="mt-5 font-semibold border-t border-b border-gray-300 p-5 text-lg">
            Product (02)
          </h3>
