...
import {
  ...
  useDeleteAProductFromCartApiMutation,
  useGetUserCartApiQuery,
...
const ShoppingCart = () => {
  ...
  const {
    ...
    isError,
  } = useGetUserCartApiQuery(userInfo ? userInfo._id : null, {
    skip: !userInfo,
  });
  ...
  const handleCartIncrease = async (productId) => {
    if (userInfo && cart) {
      ...
      if (response.data && response.data.data) {
        ...
        console.error(
          "Unexpected API response structure for increase",
          response
        );
      }
    }
  ...
  const handleDeleteAProductFromCart = async (productId) => {
    if (userInfo && cart) {
      const response = await deleteAProductFromCartApi({
        userId: userInfo._id,
        productId,
      });
      if (response.data && response.data.data) {
        ...
        {cart?.cartItems?.length > 0 && (
          <>
            <div className="border-2 flex flex-col justify-between sm:w-full">
              <div>
                <h2 className="sm:p-5 p-4 font-semibold sm:border-0 border-b-2">
                  Shopping Card
                </h2>
                <Table className="hidden sm:block">
                  ...
                  <TableHeader>
                    <TableRow className="bg-gray-300 pointer-events-none">
                      <TableHead className="w-[600px] text-base text-black">
                        Products
                      </TableHead>
                      <TableHead className="w-[200px] text-base text-black">
                        Price
                      </TableHead>
                      <TableHead className="w-[200px] text-base text-black">
                        Quantity
                      ...
                      <TableRow key={index}>
                        <TableCell className="font-medium">
                          <div className="flex items-center gap-2">
                            <RxCrossCircled
                              onClick={() =>
                                handleDeleteAProductFromCart(item.productId)
                              }
                        ...
                        <TableCell>
                          <div className="flex items-center border-2 px-2 py-2 gap-8">
                            <Button
                              className="text-xl"
                              onClick={() => handleCartDecrease(item.productId)}
                            >
                              −
                            </Button>
                            ...
                            <Button
                              className="text-xl"
                              onClick={() => handleCartIncrease(item.productId)}
                            >
                              +
                            </Button>
                          </div>
              ...
              <div className="flex justify-between sm:p-5 p-2 border-t-2">
                <Button
                  variant="blueBorder"
                  className="sm:uppercase rounded capitalize"
                >
            ...
            {cart && (
              <div className="lg:px-4 sm:flex lg:flex-col lg:space-y-2 mt-4 sm:mt-4 lg:mt-0 sm:gap-4 lg:gap-0 sm:w-full lg:w-[40%]">
                ...
                <div className="border-2 p-5 space-y-5 sm:block hidden w-full">
                  ...
                  <Button variant="blueBorder" className="px-4 py-3">
                    Apply Coupon
                  </Button>
                </div>
              </div>
