...
const createProfile = asyncHandler(async (req, res) => {
    ...
    const { userId, firstName, lastName, secondaryEmail, contactNumber, deliveryAddress } = req.body;
    ...
    res.status(201).json(new ApiResponse(200, createdProfile, "Profile and address processing completed successfully"));
});


const updateProfile = asyncHandler(async (req, res) => {
    const { userId, firstName, lastName, secondaryEmail, contactNumber, deliveryAddress, addressId } = req.body;
    ...
    res.status(200).json(new ApiResponse(200, profile, "The profile updated successfully"));
});
...
const deleteUserProfile = asyncHandler(async(req, res) => {
    ...
    if (addressId) {
        const addressIndex = profile.deliveryAddress.findIndex(item => item._id.equals(addressId));
        ...
        res.status(201).json(
            new ApiResponse(200, profile, "One delivery address successfully deleted")
        );
    } else {
        ...
        res.status(201).json(
            new ApiResponse(200, null, "Profile successfully deleted")
        );
    }
});
