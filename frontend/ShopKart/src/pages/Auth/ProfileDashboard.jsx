...
const ProfileDashboard = () => {
  ...
  const { data: userOrderStatisticsData, isLoading: isOrderStatistics } =
    useGetUserOrderStatisticsQuery({ orderedBy: userInfo._id });
  const { data: getAllUserOrdersData, isLoading: isUserOrderLoading } =
    useGetAllUserOrdersQuery({ orderedBy: userInfo._id });
    ...
    <section>
      <main className="sm:w-[80%] my-10 mx-2">
        <h1 className="text-lg font-semibold">
          Hello,{" "}
          {userProfile?.firstName ? userProfile?.firstName : userInfo.username}
        </h1>
        <p className="sm:w-[50%]">
          From your account dashboard. you can easily check & view your{" "}
          <span className="text-blue-400 font-semibold">Recent Orders</span>,
          manage your{" "}
          <span className="text-blue-400 font-semibold">
            Delivery Addresses
          </span>{" "}
          and edit your{" "}
          <span className="text-blue-400 font-semibold">Password</span> and{" "}
          <span className="text-blue-400 font-semibold">Account Details</span>.
        </p>
        <div className="sm:flex space-y-4 sm:space-y-0 gap-4 sm:mt-10 mt-4">
          {userProfile?.firstName &&
            ...
            userProfile?.contactNumber && (
              <section className="border border-1 border-gray-300 w-full">
                <h1 className="p-4 border-b-2 font-semibold uppercase">
                  Account Info
                </h1>
                <div className="sm:p-4 p-2">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p>
                        {userProfile?.deliveryAddress[0]?.state} -{" "}
                        {userProfile?.deliveryAddress[0]?.postalCode},{" "}
                        {userProfile?.deliveryAddress[0]?.country}
                      </p>
                    </div>
                  </div>
                  <h4>
                  ...
                  <Button
                    ...
                    className="border-2 border-blue-200 bg-white text-blue-400 hover:bg-gray-50 font-semibold mt-2"
                  >
                    EDIT ACCOUNT
                  </Button>
                </div>
          ...
          {userProfile?.firstName &&
            ...
            userProfile?.deliveryAddress?.length > 0 && (
              <section className="border border-1 border-gray-300 w-full">
                <h1 className="p-4 border-b-2 font-semibold uppercase">
                  Delivery address
                </h1>
                <div className="sm:p-4 p-2">
                  <h3 className="font-semibold">
                    {userProfile?.deliveryAddress[0]?.fullName}
                  </h3>
                  <p>
                    {userProfile?.deliveryAddress[0]?.address},{" "}
                    {userProfile?.deliveryAddress[0]?.city} -{" "}
                    {userProfile?.deliveryAddress[0]?.postalCode},{" "}
                    {userProfile?.deliveryAddress[0]?.country}
                  </p>
                  <h4>
                    Phone Number:{" "}
                    <span>{userProfile?.deliveryAddress[0]?.phoneNumber}</span>
                  </h4>
                  <h4>
                    Email:<span>{userProfile?.deliveryAddress[0]?.email}</span>
                  </h4>
                  <Button
                    onClick={() => navigate("/profile/address")}
                    className="border-2 border-blue-200 bg-white text-blue-400 hover:bg-gray-50 font-semibold uppercase mt-2"
                  >
                    Edit Address
                  </Button>
                </div>
            ...
            (userProfile?.deliveryAddress ||
              ...
              <section className="flex flex-col justify-between gap-4 w-full">
                <div className="flex items-center gap-4 bg-sky-200 p-4">
                  ...
                  <div>
                    <span className="font-semibold">
                      {userOrderStatistics.totalOrders < 10
                        ? `0${userOrderStatistics.totalOrders}`
                        : userOrderStatistics.totalOrders || 0}
                    </span>
                    <p>Total Order</p>
                  </div>
                </div>
                <div className="flex items-center gap-4 bg-orange-100 p-4">
                  ...
                  <div>
                    <span className="font-semibold">
                      {userOrderStatistics.pendingOrders < 10
                        ? `0${userOrderStatistics.pendingOrders}`
                        : userOrderStatistics.pendingOrders || 0}
                    </span>
                    <p>Pending Orders</p>
                  </div>
                </div>
                <div className="flex items-center gap-4 bg-green-100 p-4">
                  ...
                  <div>
                    <span className="font-semibold">
                      {userOrderStatistics.completedOrders < 10
                        ? `0${userOrderStatistics.completedOrders}`
                        : userOrderStatistics.completedOrders || 0}
                    </span>
            ...
            (userProfile?.deliveryAddress ||
              ...
              <section className="flex items-center justify-center w-full bg-sky-100 sm:py-16 py-4 border border-1 border-gray-300">
                <h2 className="text-lg">
                  Please complete your profile setup like{" "}
                  <span
                    onClick={() => navigate("/profile/address")}
                    ...
                    {userOrders?.slice(0, 5)?.map((order, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell
                          ...
                          className="text-right flex items-center justify-end gap-2 text-blue-400 font-semibold hover:cursor-pointer"
                        >
                          View Details
              ...
              <>
                {userOrders?.slice(0, 5)?.map((order, index) => (
                  <div
                    key={index}
                    onClick={() => handleViewOrderDetails(order._id)}
                    className="sm:hidden block p-2 border-t border-solid border-gray-300 cursor-pointer"
                  >
                    <div>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="sm:text-xl text-base font-semibold text-blue-400">
                        ${order?.totalPrice}
                      </span>
