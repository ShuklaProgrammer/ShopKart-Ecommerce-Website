...
const createProduct = asyncHandler(async(req, res) => {
    ...
    const createdProduct = await Product.findById(product._id).populate("brand").populate("category").populate("reviews")
...
const addDiscountToAProduct = asyncHandler(async(req, res) => {
    ...
    const {discountName, discountType, discountValue, discountExpiry} = req.body

    if(!discountName || !discountType || !discountValue){
        throw new ApiError(404, "All the field are required for adding discount to a product")
    }
    ...
    res.status(201).json(
        new ApiResponse(200, product, "The discount added to the product successfully")
    )
})
...
const updateExpireDiscounts = asyncHandler(async(req, res) => {
    ...
    for (const product of products) {
        ...
        if (updateNeeded) {
            ...
            await product.save();
            console.log(`Updated product: ${product.title}, New Discounted Price: ${product.discountedPrice}`);
        }
    }
...
const deleteADiscountFromProduct = asyncHandler(async(req, res) => {
    ...
    const findDiscountIndex = product.discount.findIndex(discount => discount._id.toString() === discountId)

    if(findDiscountIndex === -1){
        res.status(201).json(
            new ApiResponse(200, "The hvfh")
        )
    }
    ...
    res.status(201).json(
        new ApiResponse(200, product, "The discount removed successfully")
    )
})
                ...
                reviews: {
                    $map: {
                        ...
                        in: {
                            ...
                            commenterName: {
                                $cond: {
                                    if: { $ne: [{ $indexOfArray: ["$commenters.userId", "$$review.userId"] }, -1] },
                                    then: {
                                        $concat: [
                                            { $ifNull: [{ $arrayElemAt: ["$commenters.firstName", { $indexOfArray: ["$commenters.userId", "$$review.userId"] }] }, "Anonymous"] },
                                            " ",
                                            { $ifNull: [{ $arrayElemAt: ["$commenters.lastName", { $indexOfArray: ["$commenters.userId", "$$review.userId"] }] }, "User"] }
                                        ]
                                    },
                                    else: "Anonymous User"
                                }
                            ...
                            commenterImage: {
                                $cond: {
                                    if: { $ne: [{ $indexOfArray: ["$commenters.userId", "$$review.userId"] }, -1] },
                                    then: { $ifNull: [{ $arrayElemAt: ["$commenters.profileImage", { $indexOfArray: ["$commenters.userId", "$$review.userId"] }] }, "defaultImage.png"] },
...
const deleteProductById = asyncHandler(async(req, res) => {
    ...
    res.status(201).json(
        new ApiResponse(200, product, "The product deleted successfully.")
    )    
})

const getAllProduct = asyncHandler(async(req, res) => {

    const {filterCategory: categoryName, filterBrand: brandName, filterTags, filterPrice, sort, search, searchSuggestion, page, limit}= req.query
    ...
    if(categoryName){
        const categories = await Category.find({ categoryName: {$in: categoryName.split(",")}}, "_id");
        if (!categories || categories.length === 0) {
            throw new ApiError(404, "Category not found");
        }

        query = query.find({ category: {$in: categories.map(category => category._id)} });
    }

    if(brandName){
        const brands = await Brand.find({brandName: {$in: brandName.split(",")}}, "_id")
    ...
    //search suggestion
    if(searchSuggestion){
        const suggestion = await Product.find({title: {$regex: suggestion, $options: "i"}}).limit(10).select("title")
        res.status(201).json(
            new ApiResponse(200, suggestion, "You got the suggestion for the searching of the product")
        )
        return
    }
    ...
    res.status(201).json(
        new ApiResponse(200, {products, page, totalProducts, totalPages}, "You got all the products successfully.")
    )
})

export {createProduct, addDiscountToAProduct, updateExpireDiscounts, updateProduct, getProductById, deleteProductById, deleteADiscountFromProduct, getAllProduct}