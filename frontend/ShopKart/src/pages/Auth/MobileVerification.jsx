      ...
      <main className="bg-white shadow-2xl w-full mx-4 sm:mx-0 sm:w-[30%] pb-6">
        ...
        >
          <p className="text-center">
            Please enter your mobile number to proceed. We'll send a
          ...
          <Button type="submit" variant="shop" disabled={formik.isSubmitting}>
            {formik.isSubmitting ? (
...
const VERIFYCODE = ({ phoneNumber }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        const response = await verifyMobileCode({
  ...
  return (
    <section className="flex justify-center my-10">
      <main className="bg-white shadow-2xl w-full mx-4 sm:mx-0 sm:w-[30%] pb-6">
        ...
        >
          <p className="text-center">
            To verify your mobile number, please enter the verification code
            sent via SMS.
          </p>
          <div className="w-full my-4 space-y-2">
            <label
...
const MobileVerification = () => {
  const [isClickedOnSendCodeButton, setIsClickedOnSendCodeButton] =
    useState(false);
  ...
  return (
    <section>
      {isClickedOnSendCodeButton ? (
        <VERIFYCODE phoneNumber={phoneNumber} />
      ) : (
        <SENDCODE sendCodeClick={handleSendCodeClick} />
      )}
    </section>
  );
};
