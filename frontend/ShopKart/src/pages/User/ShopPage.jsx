...
import { updateBrand, updateCategory, updateSearch, updateTags, updatePrice } from '@/redux/features/product/productSlice'
...
const ShopPage = () => {
    ...
    const updateURL = debounce(() => {
        ...
        if (selectCategory.length > 0) params.set("category", selectCategory.join(","))
    ...
    useEffect(() => {
        ...
        dispatch(updatePrice(params.get("priceRanges")?.split(",") || (params.get("price")?.split("-") || [0, 10000])))
    }, [location.search])
    ...
    const handleCategoryChange = (categoryName) => {
        setSelectCategory(prevCategory => (
            prevCategory.includes(categoryName) ? prevCategory.filter(cat => cat !== categoryName) : [...prevCategory, categoryName]
        ))
    }

    const handleBrandChange = (brandName) => {
        setSelectBrand(prevBrand => (
            prevBrand.includes(brandName) ? prevBrand.filter(br => br !== brandName) : [...prevBrand, brandName]
        ))
    }

    const handleTagChange = (tag) => {
        setTags(prevTag => (
            prevTag.includes(tag) ? prevTag.filter(tag => tag !== tag) : [...prevTag, tag]
        ))
    }

    const handlePriceRangeChange = (priceRange) => {
        setSelectedPriceRanges(prevPriceRange => (
            prevPriceRange.includes(priceRange) ? prevPriceRange.filter(range => range !== priceRange) : [...prevPriceRange, priceRange]
        ))
    }
