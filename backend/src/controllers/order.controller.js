...
const addOrder = asyncHandler(async (req, res) => {
  ...
  // Calculate subtotal for order
  order.subTotal = orderItemsData.reduce(
...
const trackOrder = asyncHandler(async (req, res) => {
  ...
  res
    .status(201)
    .json(new ApiResponse(200, order, "Tracking order by id is successful"));
});
...
const getAllOrder = asyncHandler(async (req, res) => {
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        { orders, page, totalPages, totalOrders },
        "You got all orders successfully"
      )
    );
});
          ...
          $arrayElemAt: [
            {
              $filter: {
                ...
                as: "address",
                cond: {
                  $eq: ["$$address._id", { $toObjectId: "$deliveryAddress" }],
                },
              },
            },
            0,
...
const updateOrderById = asyncHandler(async (req, res) => {
  ...
  res
    .status(201)
    .json(new ApiResponse(200, order, "The order is updated successfully"));
});
...
const deleteOrderById = asyncHandler(async (req, res) => {
  ...
  res
    .status(201)
    .json(new ApiResponse(200, order, "The order deleted successfully"));
});
...
const getUserOrder = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(
      new ApiResponse(200, userOrder, "You get the user order successfully")
    );
});
...
const getAllUserOrders = asyncHandler(async (req, res) => {
  ...
  const userAllOrders = await Order.find({ orderedBy })
    .sort({ createdAt: -1 })
    .skip(skip)
    .limit(limit);
  ...
  res.status(201).json(
    new ApiResponse(
      ...
      "You get all the orders of the user sucessfully"
    )
  );
});
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        stats[0],
        "You get user order statistics successfully"
      )
    );
});
...
const orderStatistics = asyncHandler(async (req, res) => {
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        { totalOrders, totalIncome, totalUsers, salesByMonth },
        "You get the order statistics successfully"
      )
    );
});
    ...
    },
    {
      $group: {
        ...
        states: {
          $push: {
            state: "$_id",
            totalSalesByState: "$totalSalesByState",
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        topSales[0],
        "You get the top sales by states successfully"
      )
    );
});
