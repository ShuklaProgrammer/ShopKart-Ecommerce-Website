...
const updateDiscount = asyncHandler(async(req, res) => {
    ...
    if(!discountName || !discountType || !discountValue || !beforePrice || !afterPrice){
        throw new ApiError(400, "Please fill all the fields for updation related to discount")
    }

    const discount = await Discount.findById(discountId)

    const updateDiscount = await Discount.findByIdAndUpdate(discount._id,
         {
            discountName, 
            discountType,
            discountValue, 
...
const getDiscountById = asyncHandler(async(req, res) => {
    ...
    res.status(201).json(
        new ApiResponse(200, discount, "You get the discount by its Id")
    )
})
...
const deleteDiscountById = asyncHandler(async(req, res) => {
    ...
    res.status(201).json(
        new ApiResponse(200, discount, "The discount deleted successfully")
    )
})
...
const getAllDiscount = asyncHandler(async(req, res) => {
    ...
    res.status(201).json(
        new ApiResponse(200, discount, "You got all the discounts")
    )
})


export {createDiscount, updateDiscount, getDiscountById, deleteDiscountById, getAllDiscount}