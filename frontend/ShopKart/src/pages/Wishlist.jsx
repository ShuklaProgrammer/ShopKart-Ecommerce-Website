...
const Wishlist = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetUserWishlistQuery(userInfo ? userInfo._id : null, {
    skip: !userInfo,
  });
  ...
  const handleRemoveOneWishlist = async (productId) => {
    if (userInfo && wishlist) {
      const response = await removeProductInWishlist({
        userId: userInfo._id,
        productId,
      });
      const cartData = response.data.data;
                  ...
                  <TableRow key={index}>
                    <TableCell className="font-medium flex items-center gap-2">
                      <img
                        ...
                        className="w-10 h-10"
                      />
                      <span className="line-clamp-2 w-96">
                        {item.productName}
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="flex items-center gap-2">
                      ...
                      <RxCrossCircled
                        onClick={() => handleRemoveOneWishlist(item.productId)}
              ...
              <div key={index} className="col-span-1 flex flex-col">
                ...
                <div className="flex items-center justify-between gap-2 mx-4">
                  <Button
                    onClick={() => handleAddToCart(item.productId)}
                    variant="shop"
                    className="px-5 py-3 mb-4 mt-2 w-full"
                  >
                    Add To Cart
                    <FiShoppingCart className="text-xl ml-2" />
                  </Button>
